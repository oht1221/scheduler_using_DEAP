...
def pre_evaluate(standard, CNCs, job_pool, valve_pre_CNCs, LOK_forging_CNCs,
    ...
    for i, cnc in enumerate(CNCs, 0):
        ...
        #machines[cnc_number].plusInitialTime(initial_times[cnc_number])
    ...
    for job in indiv_ref:
        if job.isSeperated() is True:
            workno = job.getWorkno()
            diff = 0
            for j in indiv_ref:
                if j.isDelayed(): continue
                if j.getWorkno() == workno: #같은 작업지시서 해당하는  job에 대해 모두 검사
                    components = j.getComponents()
